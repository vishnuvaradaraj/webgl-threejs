...
def main(argv=None):
	...
	parser.add_argument('--output', default='../js/parabaycinema-compiled.js')
	...
	if args.minify is False:
		...
		source = ' '.join(sources)
		# --compilation_level WHITESPACE_ONLY --warning_level=VERBOSE 
		cmd = 'java -jar compiler/compiler.jar  --compilation_level SIMPLE_OPTIMIZATIONS --jscomp_off=globalThis --externs %s --jscomp_off=checkTypes --language_in=ECMASCRIPT5_STRICT --js %s --js_output_file %s %s' % (externs, source, output, sourcemapargs)
		os.system(cmd)

		# header

		with open(output,'r') as f: text = f.read()
		with open(output,'w') as f: f.write('// parabaycinema.js - http://www.parabay.com\n' + text[13:] + sourcemapping)
